...
export function registerSslTools(server, ctx) {
    server.registerTool("ssl_describe_certificates", {
    ...
    server.registerTool("ssl_describe_certificate_detail", {
    ...
    server.registerTool("ssl_apply_certificate", {
        description: "申请免费 DV 证书（ApplyCertificate）。默认 PackageType=83（TrustAsia C1 DV Free）、DvAuthMethod=DNS_AUTO。需 confirm=true",
        inputSchema: {
        ...
        if (!confirm) {
            return textResult(rejectBlocked("ssl", "apply_certificate", "申请证书需显式设置 confirm=true"));
        }
        const client = createSslClient(ctx.getCredentials(), region);
        return safeResult(await client.ApplyCertificate({
            DvAuthMethod: dvAuthMethod ?? "DNS_AUTO",
            DomainName: domainName,
            PackageType: packageType ?? "83"
        }));
    });
    server.registerTool("ssl_describe_host_cos_instances", {
        description: "列出证书可部署的 COS 自定义域名实例（DescribeHostCosInstanceList）。不要传 Region 参数，会报 UnknownParameter",
        inputSchema: {
            certificateId: z.string(),
            region: z.string().optional()
        }
        ...
        const client = createSslClient(ctx.getCredentials(), region);
        return safeResult(await client.DescribeHostCosInstanceList({
            CertificateId: certificateId,
            ResourceType: "cos"
        }));
    });
    server.registerTool("ssl_deploy_certificate_instance", {
        description: "把证书部署到云资源（DeployCertificateInstance）。COS 的 InstanceIdList 必须是 Region|Bucket|Domain。需 confirm=true",
        inputSchema: {
            certificateId: z.string(),
        ...
        if (!confirm) {
            return textResult(rejectBlocked("ssl", "deploy_certificate_instance", "部署证书需显式设置 confirm=true"));
        }
        const kind = resourceType ?? "cos";
        if (kind === "cos") {
            for (const item of instanceIdList) {
                if (item.split("|").length !== 3) {
                    throw new Error(`COS 部署 InstanceIdList 必须是 Region|Bucket|Domain，收到 ${item}`);
                }
            }
        }
        const client = createSslClient(ctx.getCredentials(), region);
        return safeResult(await client.DeployCertificateInstance({
            CertificateId: certificateId,
            InstanceIdList: instanceIdList,
            ResourceType: kind
        }));
    });
    server.registerTool("ssl_describe_host_deploy_record_detail", {
        ...
        const client = createSslClient(ctx.getCredentials(), region);
        return safeResult(await client.DescribeHostDeployRecordDetail({
