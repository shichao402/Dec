...
| 四象限资产视图（`QuadrantAsset`）与提供方 `depends_on` | MCP JSON / TOML 片段正文 |
| `project_name`、消费声明 `requires` | Bitwarden 私密正文 |

## 顶层项目与四象限

- **Project**（`projects.proto`）：vault 顶层 `<name>/dec.yaml`；名称强制小写 kebab-case。`tags` 当前可配置 `global`（推荐作为 Global 资产导入本机）。
