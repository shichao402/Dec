...
**给全局配置补 `version` 字段，避免被误认成 v1 项目配置。** 当时否决的是「只用 version 当防误认」。现在全局配置**应当**带 `kind` + `version`（见 [0017](0017-local-layout-version.md)）；防误认靠 `kind` 与空 root 守卫，不靠 version 数字兼容。
