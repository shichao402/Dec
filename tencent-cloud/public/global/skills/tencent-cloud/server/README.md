...
单一高权限腾讯云 MCP：CVM、COS、DNSPod、SSL、Lighthouse、CDB MySQL 通过 SDK 直连。**敏感销毁/全开放安全组等操作已禁用。**
...
| **cos** (12) | `list_objects`, `upload_object`, `download_object`, `head_object`, `delete_object`†, `get_bucket_info`, `get_bucket_cors`, `put_bucket_cors`, `get_bucket_policy`, `put_bucket_policy`, `get_bucket_domain`, `put_bucket_domain`† |
| **dns** (5) | `describe_domains`, `describe_records`, `create_record`, `modify_record`, `delete_record` |
| **ssl** (6) | `describe_certificates`, `describe_certificate_detail`, `apply_certificate`†, `describe_host_cos_instances`, `deploy_certificate_instance`†, `describe_host_deploy_record_detail` |
...
† `cos_delete_object`、`cos_put_bucket_domain`、`ssl_apply_certificate`、`ssl_deploy_certificate_instance`、`cdb_close_wan_service`、`cdb_add_security_group_ingress` 需 `confirm=true`  
...
运行 `meta_list_blocked_operations` 查看完整列表。主要包括：
