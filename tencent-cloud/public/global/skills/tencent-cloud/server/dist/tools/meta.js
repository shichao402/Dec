...
export function registerMetaTools(server, ctx, meta) {
    server.registerTool("meta_list_namespaces", {
    ...
    server.registerTool("meta_list_blocked_operations", {
        description: "列出已禁用的敏感操作及安全策略说明",
        inputSchema: {}
    }, async () => textResult(listBlockedOperations()));
    server.registerTool("meta_sync_pkv_config", {
    ...
    server.registerTool("meta_sync_dec_assets", {
