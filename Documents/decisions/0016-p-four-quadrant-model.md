...
Git 仓库每个合法顶层目录都是一个项目；项目可以对应代码项目，也可以只是通用能力包。
...
两个平面的敏感正文，不建立多余的 public folder。同一 项目/plane/相对路径 不得同时由
Git 与 BW 持有。

Bitwarden 的 folder 只有一层，名字里的斜杠不表示层级。因此 folder 名就是项目名，
平面与同步根相对路径一起编码进条目名（`private/<plane>/<rel>`，SSH Key 条目也不
例外）。`<p>/private/<plane>` 只是逻辑地址写法；这套切分只在 BW 实现内部定义，
`internal/app` 与 `internal/tui` 只传 (项目, 平面, 相对路径)。
...
- project 与 user 的 SSH 密钥文件名分域，同一 项目/实例 不得相互覆盖或撤销。

阶段 1–3 只切换 Git 资产路径；在新的 BW 协议完成前，项目仓库不得误用旧
`bundle/<name>` 协议同步 secrets。

### 4. 写入与冲突

项目平面只能回推本仓项目，不能把 requires 引入的副本写回依赖项目。用户平面可回推本机
显式启用的项目。多个项目在同一平面竞争同一 IDE 目标路径时硬失败；不得按扫描顺序覆盖。
...
**长期新旧双读。** 同一资产可能从两棵树读写，无法定义权威来源。远端一次性迁完后只读项目模型。
...
- 项目名按小写 kebab-case 规范化；仅大小写不同的旧名称合并为同一项目。缺失 requires 与 user-scope 引用降为警告并丢弃。
