...
- **用户级启用**（[0003](0003-user-enabled-secret-bundles.md)）：可在本机 Settings 全局启用 Dec bundle（公开资产 ∪ secrets），不必经各 project `enabled_bundles`；不新增 `user/` folder 协议；secrets-only 在启用保存时补 vault 占位
