...
Dec CLI 的检查/下载已切换到 `cnb.cool/shichao402/relkit/sdk`，发布走腾讯云 COS 自有域名。
...
| 私钥（本地 SSOT） | `.secrets/bundles/relkit/keys/dec-2026.private.pb` | 相对 **含 `relkit.json` 的项目根**（relkit 用配置文件所在目录解析 `privateKeyPath`，与进程 cwd 无关） |
| Bitwarden | folder `bundle/relkit`，Note 名 `keys/dec-2026.private.pb` | pull 后落到上述本地路径 |
