...
export function registerDnsTools(server, ctx) {
    server.registerTool("dns_describe_domains", {
    ...
    server.registerTool("dns_describe_records", {
        ...
        const params = {
            Limit: limit ?? 20,
            Offset: offset ?? 0,
            ...(domainId !== undefined ? { DomainId: domainId } : { Domain: domain }),
            ...(subDomain ? { SubDomain: subDomain } : {}),
            ...(recordType ? { RecordType: recordType } : {})
        };
        return safeResult(await client.DescribeRecordList(params));
    });
    server.registerTool("dns_create_record", {
        ...
        const client = createDnspodClient(ctx.getCredentials());
        return safeResult(await client.CreateRecord({
            Domain: domain,
    ...
    server.registerTool("dns_modify_record", {
    ...
    }, async ({ domain, recordId, subDomain, recordType, recordLine, value, ttl, mx, status }) => {
        const client = createDnspodClient(ctx.getCredentials());
        return safeResult(await client.ModifyRecord({
            Domain: domain,
            RecordId: recordId,
            RecordType: recordType,
            RecordLine: recordLine,
    ...
    server.registerTool("dns_delete_record", {
        ...
        if (!domain?.trim() || recordId === undefined || recordId <= 0) {
            return safeResult(rejectBlocked("dns", "delete_record", "删除记录必须同时提供有效 domain 与 recordId"));
        }
        const client = createDnspodClient(ctx.getCredentials());
