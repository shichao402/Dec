...
// BindManagedProject 将服务器目录显式绑定到仓库中的本仓项目。
...
func BindManagedProject(projectRoot, projectName string) (*BindManagedProjectResult, error) {
	...
	if projectRoot == "" || projectName == "" {
		return nil, fmt.Errorf("项目路径与本仓项目不能为空")
	}
	...
	if len(available) > 0 {
		if !types.IsValidPName(projectName) {
			return nil, fmt.Errorf("本仓项目名称无效: %s", projectName)
		}
		if _, ok := available[projectName]; !ok {
			return nil, fmt.Errorf("仓库中不存在本仓项目 %q", projectName)
		}
	}
