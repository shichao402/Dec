...
func TestMigrateLegacyProjectLayoutsForCodex(t *testing.T) {
	...
	notes, err := migrateLegacyProjectLayouts(projectRoot, []string{"cursor", "codex-internal"})
	if err != nil {
		t.Fatalf("迁移失败: %v", err)
	}
	if len(notes) == 0 {
...
func TestMigrateLegacyProjectLayoutsSkipsWithoutCodex(t *testing.T) {
	projectRoot := t.TempDir()
	notes, err := migrateLegacyProjectLayouts(projectRoot, []string{"cursor", "codebuddy"})
	if err != nil {
		t.Fatalf("不应报错: %v", err)
	}
	if len(notes) != 0 {
