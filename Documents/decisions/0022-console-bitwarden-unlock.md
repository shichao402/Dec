...
1. 直接启动已安装的 Console 可执行文件，或把 `--unlock-local` 交给已运行的实例；
2. 只发送固定参数 `--unlock-local`，不经过 `dec://`、xdg-open、`cmd start` 或系统浏览器，也不携带凭据、token、项目路径或业务参数；
3. 等待认证完成、用户取消或超时；
...
**A. 保留服务端临时网页，或经系统 URL 协议（`dec://` / xdg-open / `cmd start`）交给浏览器。**  
否决：形成第二个人工入口，未注册协议时还会掉进浏览器；弹窗归属、聚焦、测试和远端语义继续分裂。
