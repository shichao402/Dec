// Package pmodel loads the top-level project and four-quadrant asset model.
package pmodel
...
// Loaded is a validated project manifest plus the assets physically present in Git.
type Loaded struct {
	Manifest types.P
...
func Load(repoDir, name string) (*Loaded, error) {
	...
	if !types.IsValidPName(name) {
		return nil, fmt.Errorf("项目名 %q 非法，必须为小写 kebab-case", name)
	}
	...
	if info, err := os.Lstat(manifestPath); err == nil && info.Mode()&os.ModeSymlink != 0 {
		return nil, fmt.Errorf("项目声明 %s 不能是符号链接", manifestPath)
	}
	data, err := os.ReadFile(manifestPath)
	if err != nil {
		return nil, fmt.Errorf("读取项目声明 %s 失败: %w", manifestPath, err)
	}
	var manifest types.P
	if err := yaml.Unmarshal(data, &manifest); err != nil {
		return nil, fmt.Errorf("解析项目声明 %s 失败: %w", manifestPath, err)
	}
...
// SaveManifest validates and writes the only mutable declaration of a project.
...
func SaveManifest(repoDir string, manifest types.P) error {
	...
	if err := os.MkdirAll(filepath.Dir(manifestPath), 0o755); err != nil {
		return fmt.Errorf("创建项目 %q 目录失败: %w", manifest.Name, err)
	}
	if err := os.WriteFile(manifestPath, data, 0o644); err != nil {
		return fmt.Errorf("写入项目声明 %s 失败: %w", manifestPath, err)
	}
	for _, visibility := range []types.AssetVisibility{types.AssetVisibilityPublic, types.AssetVisibilityPrivate} {
		for _, plane := range []types.AssetPlane{types.AssetPlaneGlobal, types.AssetPlaneLocal} {
...
// Scan loads every valid top-level project. Reserved/hidden directories are ignored;
...
func Scan(repoDir string) (map[string]*Loaded, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("读取项目仓库失败: %w", err)
	}
	...
	for _, entry := range entries {
		...
		if _, err := os.Lstat(filepath.Join(repoDir, entry.Name(), types.ProjectManifestFileName)); err != nil {
			if os.IsNotExist(err) {
				return nil, fmt.Errorf("顶层目录 %q 缺少 dec.yaml：项目仓库不允许其它顶层目录", entry.Name())
			}
			return nil, err
		}
		if !types.IsValidPName(entry.Name()) {
			return nil, fmt.Errorf("包含 dec.yaml 的顶层目录 %q 非法：项目名必须为小写 kebab-case", entry.Name())
		}
