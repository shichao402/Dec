...
const WAN_STATUS_TEXT = {
    0: "未开通外网",
    1: "已开通外网",
    2: "已关闭外网"
};
function resolveInstanceId(instanceId, defaultId) {
    const id = instanceId ?? defaultId;
    if (!id)
        throw new Error("请提供 instanceId 或在 .config/mise/conf.d/tencent-cloud.toml 配置 CDB_INSTANCE_ID");
    return id;
}
function extractWanInfo(instance, instanceId) {
    const wanStatus = instance.WanStatus;
    return {
        instanceId,
        wanStatus,
        wanStatusText: wanStatus !== undefined ? (WAN_STATUS_TEXT[wanStatus] ?? `未知(${wanStatus})`) : "未知",
        wanDomain: instance.WanDomain,
        wanPort: instance.WanPort,
        vip: instance.Vip,
        vport: instance.Vport
    };
}
async function fetchWanInfo(client, instanceId) {
...
async function waitAsyncRequest(client, asyncRequestId, timeoutMs = 120_000) {
...
async function addVpcIngressRules(vpcClient, securityGroupId, cidrBlock, rules) {
    const added = [];
...
function resolveIngressRules(port, protocol, includeInternalPort, description) {
    const rules = [
...
export function registerCdbTools(server, ctx) {
    server.registerTool("cdb_describe_instances", {
        ...
        else if (creds.cdbInstanceId)
            params.InstanceIds = [creds.cdbInstanceId];
        return safeResult(await client.DescribeDBInstances(params));
    });
    server.registerTool("cdb_describe_wan_service", {
        ...
        if (!wan)
            throw new Error(`未找到实例: ${id}`);
        return safeResult(wan);
    });
    server.registerTool("cdb_open_wan_service", {
    ...
    server.registerTool("cdb_close_wan_service", {
    ...
    server.registerTool("cdb_describe_databases", {
        ...
        if (!id)
            throw new Error("请提供 instanceId 或在 .config/mise/conf.d/tencent-cloud.toml 配置 CDB_INSTANCE_ID");
        const client = createCdbClient(creds, region);
        return safeResult(await client.DescribeDatabases({
            InstanceId: id,
            Limit: limit ?? 100,
            Offset: offset ?? 0
        }));
    });
    server.registerTool("cdb_describe_accounts", {
    ...
    server.registerTool("cdb_describe_tables", {
        ...
        if (!id)
            throw new Error("请提供 instanceId 或在 .config/mise/conf.d/tencent-cloud.toml 配置 CDB_INSTANCE_ID");
        const client = createCdbClient(creds, region);
        return safeResult(await client.DescribeTables({
            InstanceId: id,
            Database: database,
            Limit: limit ?? 100,
            Offset: offset ?? 0
    ...
    server.registerTool("cdb_create_database", {
        ...
        if (!id)
            throw new Error("请提供 instanceId 或在 .config/mise/conf.d/tencent-cloud.toml 配置 CDB_INSTANCE_ID");
        const client = createCdbClient(creds, region);
        return safeResult(await client.CreateDatabase({
            InstanceId: id,
            DBName: database,
            CharacterSetName: charset ?? "utf8mb4"
        }));
    });
    server.registerTool("cdb_describe_security_groups", {
    ...
    server.registerTool("cdb_add_security_group_ingress", {
        description: "为 CDB 关联的 VPC 安全组添加入站规则（CreateSecurityGroupPolicies）。禁止 0.0.0.0/0 与全端口；外网端口默认同时放通内网 3306（WAN 代理需要），需 confirm=true",
        inputSchema: {
    ...
    }, async ({ instanceId, securityGroupId, cidr, port, protocol, description, includeInternalPort, region, confirm }) => {
        if (!confirm) {
            return textResult({
                blocked: true,
        ...
        const ruleSpecs = resolveIngressRules(portStr, proto, includeInternalPort !== false, description);
        ...
        const toAdd = ruleSpecs.filter((rule) => !existingInbound.some((r) => r.CidrIp === cidrBlock &&
        ...
        const matched = after.Groups?.find((g) => g.SecurityGroupId === sgId)?.Inbound?.filter((r) => {
            if (r.CidrIp !== cidrBlock)
                return false;
            return ruleSpecs.some((rule) => String(r.PortRange) === rule.port &&
                String(r.IpProtocol).toLowerCase() === rule.protocol.toLowerCase());
        });
        return safeResult({
            success: true,
    ...
    server.registerTool("cdb_query_sql", {
    ...
    server.registerTool("cdb_execute_sql", {
