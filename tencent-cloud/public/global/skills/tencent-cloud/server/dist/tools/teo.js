 ...
 */
export function registerTeoTools(server, ctx) {
    server.registerTool("teo_describe_zones", {
    ...
    server.registerTool("teo_verify_ownership", {
    ...
    server.registerTool("teo_check_cname_status", {
        description: "检查域名的 CNAME 是否已指向 EdgeOne 分配的地址（CheckCnameStatus）。active 才算接入成功",
        inputSchema: {
            zoneId: z.string(),
        ...
        const client = createTeoClient(ctx.getCredentials(), region);
        return safeResult(await client.CheckCnameStatus({
            ZoneId: zoneId,
            RecordNames: recordNames.map((name) => name.trim().toLowerCase())
        }));
    });
    server.registerTool("teo_apply_free_certificate", {
        description: "为 EdgeOne 域名申请免费证书（ApplyFreeCertificate）。默认 dns_challenge（DNS 委派）。需 confirm=true",
        inputSchema: {
            zoneId: z.string(),
            domain: z.string(),
        ...
        if (!confirm) {
            return textResult(rejectBlocked("teo", "apply_free_certificate", "申请证书需显式设置 confirm=true"));
        }
        const client = createTeoClient(ctx.getCredentials(), region);
        return safeResult(await client.ApplyFreeCertificate({
            ZoneId: zoneId,
            Domain: domain.trim().toLowerCase(),
            VerificationMethod: verificationMethod ?? "dns_challenge"
        }));
    });
    server.registerTool("teo_check_free_certificate_verification", {
        description: "查询免费证书申请结果（CheckFreeCertificateVerification）。报 FreeCertificateVerificationIsEmpty 说明还没触发申请",
        inputSchema: {
            zoneId: z.string(),
            domain: z.string(),
            region: z.string().optional()
        ...
        const client = createTeoClient(ctx.getCredentials(), region);
        return safeResult(await client.CheckFreeCertificateVerification({
            ZoneId: zoneId,
            Domain: domain.trim().toLowerCase()
        }));
    });
    server.registerTool("teo_modify_hosts_certificate", {
        description: "配置域名证书（ModifyHostsCertificate）。eofreecert 启用免费证书，disable 关闭。需 confirm=true",
        inputSchema: {
            zoneId: z.string(),
        ...
        if (!confirm) {
            return textResult(rejectBlocked("teo", "modify_hosts_certificate", "改域名证书需显式设置 confirm=true"));
        }
        const client = createTeoClient(ctx.getCredentials(), region);
        return safeResult(await client.ModifyHostsCertificate({
            ZoneId: zoneId,
