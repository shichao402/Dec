...
- any repository default project such as `{{VIKUNJA_PROJECT}}`
...
- treat repository defaults such as `{{VIKUNJA_PROJECT}}` as defaults only, never as universal truth
...
- keep the working plan in Vikunja by default; create `{{TASK_DOCS_DIR}}/VK-<id>.md` only when repository rules require it or the round needs durable repo-local design or verification notes
...
3. project-local Dec vars such as `{{VIKUNJA_PROJECT}}` and `{{TASK_DOCS_DIR}}`
