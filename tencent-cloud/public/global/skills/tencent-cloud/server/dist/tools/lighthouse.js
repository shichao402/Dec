...
import { buildDownloadScript, buildUploadScript, describeInvocation, runShellCommand } from "../helpers/tat.js";
import { safeResult } from "./common.js";
export function registerLighthouseTools(server, ctx) {
    server.registerTool("lighthouse_describe_instances", {
    ...
    server.registerTool("lighthouse_describe_firewall_rules", {
        ...
        const client = createLighthouseClient(ctx.getCredentials(), region);
        return safeResult(await client.DescribeFirewallRules({
            InstanceId: instanceId,
            Limit: limit ?? 100,
            Offset: offset ?? 0
        }));
    });
    server.registerTool("lighthouse_run_command", {
    ...
    server.registerTool("lighthouse_upload_file", {
        ...
        const client = createTatClient(ctx.getCredentials(), region);
        return safeResult(await runShellCommand(client, [instanceId], script, {
            commandName: "mcp-lh-upload",
            timeout: 300
        }));
    });
    server.registerTool("lighthouse_download_file", {
    ...
    server.registerTool("lighthouse_describe_invocation", {
