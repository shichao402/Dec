...
func TestRunPagePMigrationPreviewAndDoubleConfirmation(t *testing.T) {
	oldPreview := previewPMigrationOperation
	oldRun := runPMigrationOperation
	t.Cleanup(func() {
		previewPMigrationOperation = oldPreview
		runPMigrationOperation = oldRun
	})
	previewPMigrationOperation = func(context.Context, app.Workspace) (*app.PMigrationPlan, error) {
		return &app.PMigrationPlan{
			LegacyDetected: true,
			Fingerprint:    "fingerprint",
			Manifests:      []app.PMigrationManifest{{Name: "my-app"}},
			GitMoves:       []app.PMigrationGitMove{{Source: "bundles/a/rules/a.mdc", Target: "a/public/project/rules/a.mdc"}},
		}, nil
	}
	runPMigrationOperation = func(context.Context, app.Workspace, string, app.Reporter) (*app.PMigrationJournal, error) {
		return &app.PMigrationJournal{Phase: app.PMigrationComplete}, nil
	}

	m := newModel("/tmp/dec-project", "dev")
	m.pageIndex = 3 // Run
	updated, cmd := m.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'m'}})
	m = updated.(model)
	if m.migrationStage != "loading" || cmd == nil {
		t.Fatalf("m 后 stage=%q cmd=%v", m.migrationStage, cmd)
	}
	updated, _ = m.Update(cmd())
	m = updated.(model)
	if m.migrationStage != "preview" || m.migrationPlan == nil {
		t.Fatalf("preview stage=%q plan=%#v", m.migrationStage, m.migrationPlan)
	}
	updated, _ = m.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'y'}})
	m = updated.(model)
	if m.migrationStage != "confirm" {
		t.Fatalf("首次确认 stage=%q", m.migrationStage)
	}
	updated, cmd = m.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'y'}})
	m = updated.(model)
	if m.migrationStage != "running" || cmd == nil {
		t.Fatalf("二次确认 stage=%q cmd=%v", m.migrationStage, cmd)
	}
	updated, _ = m.Update(cmd())
	m = updated.(model)
	if m.migrationStage != "done" || m.migrationJournal == nil || m.migrationJournal.Phase != app.PMigrationComplete {
		t.Fatalf("done stage=%q journal=%#v", m.migrationStage, m.migrationJournal)
	}
}

func TestRunPagePMigrationBlockersCannotExecute(t *testing.T) {
	m := newModel("/tmp/dec-project", "dev")
	m.pageIndex = 3 // Run
	m.migrationStage = "preview"
	m.migrationPlan = &app.PMigrationPlan{
		LegacyDetected: true,
		Issues: []app.PMigrationIssue{{
			Code: "target_conflict", Severity: "error", Message: "目标冲突",
		}},
	}
	updated, cmd := m.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'y'}})
	m = updated.(model)
	if m.migrationStage != "preview" || cmd != nil {
		t.Fatalf("blocker 应停留 preview: stage=%q cmd=%v", m.migrationStage, cmd)
	}
	if view := m.View(); !strings.Contains(view, "阻断") || !strings.Contains(view, "目标冲突") {
		t.Fatalf("预览应结构化展示阻断:\n%s", view)
	}
}
