...
async function callPages<T>(
  ...
  const body = (parsed as { Response?: unknown }).Response ?? parsed;
...
export function registerPagesTools(server: McpServer, ctx: ToolContext) {
  const call = <T>(action: string, payload: Record<string, unknown>, region?: string) =>
    callPages<T>(ctx.projectRoot, action, payload, region);
  ...
  server.registerTool(
    ...
    async ({ name, status, projectIds, limit, offset, region }) => {
      ...
      if (name) filters.push({ Name: "Name", Values: [name] });
      if (status) filters.push({ Name: "Status", Values: [status] });
  ...
  server.registerTool(
    ...
    {
      description:
        "创建 Pages/Makers 项目（CreatePagesProject）。默认 Provider=Upload，即由本地产物直接部署，不接 Git",
      inputSchema: {
        name: z
          .string()
    ...
    async ({ name, provider, area, outputDir, buildCmd, installCmd, repoUrl, repoBranch, region }) => {
      ...
      if (buildCmd) payload.BuildCmd = buildCmd;
      if (installCmd) payload.InstallCmd = installCmd;
      if (repoUrl) payload.RepoUrl = repoUrl;
      if (repoBranch) payload.RepoBranch = repoBranch;
  ...
  server.registerTool(
    ...
    {
      description:
        "取上传产物用的 COS 临时密钥与目标路径（DescribePagesCosTempToken）。一般直接用 pages_deploy_dir",
      inputSchema: {
        projectId: z.string(),
        region: z.string().optional()
      }
  ...
  server.registerTool(
    ...
    {
      description:
        "创建部署任务（CreatePagesDeployment）。产物已经在 COS 上时才用这个，否则用 pages_deploy_dir",
      inputSchema: {
        projectId: z.string(),
        tempBucketPath: z
          .string()
  ...
  server.registerTool(
    ...
    {
      description:
        "把本地目录或 zip 部署到 Pages/Makers：取临时凭据 → 上传 COS → 创建部署。需 confirm=true",
      inputSchema: {
        projectId: z.string(),
    ...
    async ({ projectId, path, confirm, env, region }) => {
      if (!confirm) {
        return textResult(
          rejectBlocked("pages", "deploy_dir", "上传并部署需显式设置 confirm=true")
        );
      }
      ...
      const temp = await call<TempToken>(
      ...
      const tempBucketPath = asZip
        ...
        : temp.TargetPath;
      const deployment = await call<{ DeploymentId?: string }>(
        "CreatePagesDeployment",
        {
          ProjectId: projectId,
          ViaMeta: "Upload",
  ...
  server.registerTool(
    ...
    async ({ projectId, status, deploymentIds, limit, offset, region }) => {
      ...
      if (status) payload.Filters = [{ Name: "Status", Values: [status] }];
      if (deploymentIds?.length) payload.DeploymentIds = deploymentIds;
  ...
  server.registerTool(
    ...
    async ({ projectId, envVars, region }) => {
      const EnvVars = envVars.map((item) => {
        const entry: Record<string, unknown> = { Key: item.key, Value: item.value };
        if (item.comment) entry.Comment = item.comment;
        if (item.id !== undefined) entry.Id = item.id;
        return entry;
      });
      return safeResult(
        await call("ModifyPagesProjectEnvs", { ProjectId: projectId, EnvVars }, region)
      );
    }
  );
  ...
  server.registerTool(
    ...
    {
      description:
        "给项目绑自定义域名（CreatePagesZoneCustomDomain）。返回 Cname 与归属校验要求，之后用 dns_create_record 指过去",
      inputSchema: {
        projectId: z.string(),
    ...
    async ({ projectId, domain, region }) => {
      ...
      return safeResult(
        await call("CreatePagesZoneCustomDomain", { ProjectId: projectId, Domain: name }, region)
      );
    }
  );
  ...
  server.registerTool(
    ...
    async ({ projectId, domain, region }) => {
      ...
      if (domain) payload.Domain = domain.trim().toLowerCase();
