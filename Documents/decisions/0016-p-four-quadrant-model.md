...
**长期新旧双读。** 同一资产可能从两棵树读写，无法定义权威来源。迁移应有预览和恢复，
完成后只读 P 模型。

## 一次性迁移（阶段 7）

- Run 页 `m` 先只读扫描 Git 与 Bitwarden 元数据，展示名称规范化、缺失引用、目标冲突及
  Git/BW 同逻辑路径冲突；有阻断问题时不能执行。
- P 名按小写 kebab-case 规范化；多个旧名称落到同一名称时硬失败，不按扫描顺序覆盖。
- 执行顺序固定为：本地备份 → Git 新树写入并校验 → BW 新 folder 写入并校验 →
  本地配置/落地切换 → 删除旧 BW 项 → 删除旧 Git 树。
- 每个成功阶段后原子更新本地恢复日志，步骤均可重入。Git 与 BW 无法组成一个事务；
  中途失败时保留旧节点及已校验的新节点，下次从日志记录阶段继续。
- 检测到 `projects/` 或 `bundles/` 时普通 Push 拒绝并引导到 Run 页迁移；运行时不保留
  新旧结构长期双读。
