...
export const BLOCKED_OPERATIONS = {
    ...
    pages: [
        "DeletePagesProject — 删除 Pages/Makers 项目",
        "DeletePagesProjectCustomDomains — 摘掉线上自定义域名"
    ],
    teo: [
        "DeleteZone / ModifyZoneStatus — 停用或删除 EdgeOne 站点",
        "ModifyHostsCertificate Mode=disable — 关闭线上域名证书（需 confirm=true 才可调用）"
    ]
};
export function rejectBlocked(namespace, operation, reason) {
    return {
        blocked: true,
        namespace,
        operation,
...
export function assertSafeFirewallRules(rules) {
    for (const rule of rules) {
        if (isDangerousNetworkRule(rule)) {
            throw new Error("拒绝：检测到对 0.0.0.0/0 开放危险端口/协议的安全组或防火墙规则。请在控制台手动操作。");
        }
    }
}
...
const DESTRUCTIVE_SQL = /\b(DROP\s+(DATABASE|TABLE|INDEX|VIEW|SCHEMA|USER)|TRUNCATE\s+TABLE|DELETE\s+FROM\s+\w+\s*;)\b/i;
