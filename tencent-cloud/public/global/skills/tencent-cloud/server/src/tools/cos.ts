...
server.registerTool(
  ...
  server.registerTool(
    ...
    {
      description:
        "绑定自定义源站域名（get 后合并再 putBucketDomain）。Type 固定 REST，避免把下载桶改成静态网站源站。需 confirm=true",
      inputSchema: {
    ...
    async ({ domain, confirm, status, type, forcedReplacement, bucket, region }) => {
      if (!confirm) {
        return textResult(
          rejectBlocked("cos", "put_bucket_domain", "绑定自定义域名需显式设置 confirm=true")
        );
      }
      ...
      let existing: { DomainRule?: Array<{ Name?: string; Status?: string; Type?: string }> } = {};
      try {
