...
Agent 直接连本机 `dec-server` 的 Streamable HTTP `http://127.0.0.1:47654/mcp`。
Console 不再提供 Agent 网关。
...
本机 `dec-server` 缺 Bitwarden session 时会拉起尚未运行的 Console，或聚焦现有窗口，并等待 Authenticate；并发请求共享同一次认证。MCP URL 没有进程在听时就是连接失败，不会因此拉起 Console。管理远端设备时，输入仍发生在操作者当前
...
3. 初始化项目并绑定作者身份家项目，再在「订阅」面板勾选要消费的项目（官方注册表或个人私仓，写 `requires`）；
...
- 表格类主体（订阅面板）撑满高度：底部操作条贴住视口下沿，行的位置不随条数跳动。
