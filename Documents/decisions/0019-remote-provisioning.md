...
- **影响范围**：`dec-server` 设备级 provisioning、`dec-host-setup`、运行时缓存与 SSH 推送、Console 连接页、`dec-mcp` 工具面
...
**探测（只读）**：`probe_remote_host` 走 `Invoke`。检查 SSH 连通性、`uname` 得到 os/arch、`~/.dec/bin` 下运行时组件是否存在且版本一致、`git` 与 `ssh-keygen` 是否就位、`~/.dec` 可写、以及**非交互 SSH 能否拉起后台进程**（按需拉起的前提）。它同时是后续每一步的前置检查，也让 Console 在人点「部署」之前就能给出「这台机器行不行」。

探测**不再**关心 systemd user / launchd / linger 状态——远端不做常驻化，这些信息对置备决策已无影响。

**安装**：`provision_remote_host` 走 `RunOperation`。发起端 `dec-server` 根据 probe 的 `os/arch` 解析 Console 钉死版本的运行时套件：先查 `~/.dec/runtime-cache/<version>/<os>-<arch>/`；缓存缺失时在发起端请求签名 RUP，并且只在 RUP 解析结果恰好等于 Console 版本时下载。同平台缓存由 Console bundle 预热。随后用系统 SSH 把四个文件流到目标临时目录，校验、chmod 后切换到 `~/.dec/bin`。目标机不执行联网安装脚本，不需要 curl、bash 或公网访问。

**配置**：不在发起端手写远端 YAML。单用途 `dec-host-setup` 在目标机本地执行，用现成的 config 包幂等写入 `management_listen`（保留其余字段）。**「装二进制」由发起端缓存 + SSH 推送负责，「写配置」由目标端脚手架负责**——这样配置合并逻辑只有一份实现，不会因远端 schema 升级（0017）而漂移。

`dec-host-setup` 不是用户面 CLI，也不承担 SSH 编排；无参执行完成固定端口配置，`--version` 只报告自身版本。根 `dec` CLI 不再为内部命令提供容器。
...
- **产物完整性校验**。同平台 Console bundle 携带 `runtime-manifest.json`；跨平台下载由 RUP 签名 manifest 与 artifact sha256 校验。持久缓存每次命中都重验清单和运行时组件摘要，半成品只存在于临时目录。
...
3. ~~`dec-host-setup`（`management_listen` 幂等写入）+ SSH 按需拉起与就绪等待~~ — **已实施**（`cmd/dec-host-setup/`、`internal/config/management_listen.go`、`internal/app/remote_service.go`）
...
- **传输后校验与回滚**：目标有 `sha256sum` 或 `shasum -a 256` 时逐件核对发起端摘要；两者都没有时降级为逐组件 `--version` 校验并明确警告。全部 stage 校验通过后才备份旧运行时套件并激活；任一步失败按已安装/已备份清单 best-effort 回滚，避免留下混合版本。
...
- **远端命令走绝对路径 `${DEC_HOME:-$HOME/.dec}/bin/<component>`**：非交互 SSH 的 PATH 通常不含 `~/.dec/bin`。
...
| 新建通用 `dec` 用户面命令族（`service setup` / `service status`） | 违反 console-first；目标端配置由无用户命令树的单用途 `dec-host-setup` 完成 |
...
- 发布流程：每个原生 Console job 生成同平台运行时套件及内置摘要清单；全平台 runtime artifact 继续发布给跨平台 RUP 缓存
