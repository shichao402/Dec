...
export function resolveProjectRoot() {
    const fromEnv = process.env.TENCENT_CLOUD_MCP_PROJECT_ROOT?.trim() ||
...
export function resolveMiseSecretsPath(projectRoot) {
...
export function defaultMiseSecretsWritePath(projectRoot) {
    return join(projectRoot, MISE_CONF_D_REL);
}
export function loadEnvFromMiseLocal(projectRoot) {
...
export function resolveCredentials(projectRoot) {
    const fromFile = loadEnvFromMiseLocal(projectRoot);
    const env = { ...fromFile, ...pickProcessEnv() };
    const secretId = env.TENCENTCLOUD_SECRET_ID ?? env.COS_SECRET_ID ?? env.TencentAPI_SecretId ?? "";
    const secretKey = env.TENCENTCLOUD_SECRET_KEY ?? env.COS_SECRET_KEY ?? env.TencentAPI_SecretKey ?? "";
    if (!secretId || !secretKey) {
        throw new Error("缺少腾讯云密钥。请配置 .config/mise/conf.d/tencent-cloud.toml 或运行 meta.sync_pkv_config 同步 pkv 密钥。");
    }
...
export function resolveEnv(projectRoot) {
 ...
 */
export function resolvePagesCredentials(projectRoot, region) {
    const env = resolveEnv(projectRoot);
    const token = (env.EDGEONE_PAGES_API_TOKEN ??
        env.EDGEONE_API_TOKEN ??
        env.PAGES_API_TOKEN ??
        env.EdgeOnePages_ApiToken ??
        "").trim();
    if (!token) {
        throw new Error("缺少 EdgeOne Pages API Token。请在 Pages 控制台 API Token 页创建后写入 .config/mise/conf.d/tencent-cloud.toml 的 EDGEONE_PAGES_API_TOKEN（腾讯云 SecretId/Key 无法调用 Pages 接口）。");
    }
...
function buildMiseLocalToml(env) {
    const keys = [
        "TENCENTCLOUD_SECRET_ID",
        "TENCENTCLOUD_SECRET_KEY",
        "TENCENTCLOUD_REGION",
...
function normalizeTencentEnv(raw) {
    const secretId = raw.TENCENTCLOUD_SECRET_ID ?? raw.COS_SECRET_ID ?? raw.TencentAPI_SecretId ?? "";
    const secretKey = raw.TENCENTCLOUD_SECRET_KEY ?? raw.COS_SECRET_KEY ?? raw.TencentAPI_SecretKey ?? "";
    return {
...
export function syncPkvConfig(projectRoot, folder = "tencent-cloud") {
    let source = "fallback";
    let rawEnv = {};
    try {
    ...
    if (!env.TENCENTCLOUD_SECRET_ID || !env.TENCENTCLOUD_SECRET_KEY) {
        throw new Error("无法获取腾讯云密钥，请解锁 Bitwarden 或检查 pkv / .config/mise/conf.d/tencent-cloud.toml");
    }
...
export function syncDecAssets(projectRoot) {
...
export function listBlockedOperations() {
    return {
        ...
        notes: {
            ...
            pages_token: "Pages/Makers 用控制台签发的 EDGEONE_PAGES_API_TOKEN，与 TENCENTCLOUD_SECRET_* 是两套凭据；证书与 CNAME 那半套走 teo_* 工具，用的才是 CAM 密钥",
            pages_delete: "DeletePagesProject / DeletePagesProjectCustomDomains 未暴露为 MCP tool；删项目或摘域名请用控制台",
            pages_deploy: "pages_deploy_dir 需 confirm=true；单次上传上限 2000 个文件 / 64MiB",
            teo_certificate: "teo_apply_free_certificate 与 teo_modify_hosts_certificate 需 confirm=true；Mode=disable 会关掉线上域名的证书",
            cdb_sql: "cdb_execute_sql 禁止 DROP/TRUNCATE/无 WHERE 的 DELETE",
            cdb_close_wan: "cdb_close_wan_service 需 confirm=true",
            cdb_security_group_ingress: "cdb_add_security_group_ingress 需 confirm=true；禁止 0.0.0.0/0 与全端口；非 3306 外网端口默认同时放通 TCP 3306",
            cdb_account_password: "ModifyAccountPassword 未暴露为 MCP tool；修改 DB 密码请用控制台或 CDB API（CDB_PASSWORD 为 MySQL 账号密码，非 TENCENTCLOUD_SECRET_*）",
...
export function cosBucketOrThrow(creds, bucket) {
...
export function promisifyCos(cos, method, params) {
