...
- **状态**：已接受（连接与发布协议已实施；GitHub Actions 原生 GUI runner 已接入；同平台运行时套件内置 + SSH 推送已接入。relkit-serve 人页按 audience 过滤仍待发布端升级）
- **日期**：2026-09-01
- **关联**：[0018](0018-instance-lock-and-console.md)、[0019](0019-remote-provisioning.md)、[0020](0020-retire-tui.md)
- **影响范围**：Console 连接、版本门闩、目标端运行时套件、RUP artifact audience、人类发布页
...
- Console 高于本机服务：用本机 listen token 停旧服务，按 Console 版本安装完整运行时套件，再拉起并复验；
- Console 高于 SSH 目标：探测后自动置备完整运行时套件；已安装设备升级无需重复确认，首次注入仍须用户显式确认；
...
每个平台的 Console 安装包内置**同 os/arch** 运行时套件（`resources/runtime/<os>-<arch>/` + `runtime-manifest.json` sha256）。本机首次连接或版本升级时从 `AppHandle.path().resource_dir()` 定位资源，校验摘要后以临时文件 + rename 原子释放到 `~/.dec/bin/`；Windows 先 rename-aside 并在失败时回滚。不再跑联网 `install.sh` / `install.ps1`，且拒绝从较新运行时降级的门闩保持不变。升级成功后删除旧 `dec` CLI。
...
**把所有平台运行时套件塞进每个 Console 安装包。** 否决：跨平台远端会迫使 Windows Console 携带所有 Linux/macOS 架构，包体巨大且仍会过期。同平台内置 + 发起端缓存推送已覆盖离线目标场景。
